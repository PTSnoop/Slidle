...
AutoViewport.setDimensions = function(requiredWidth, requiredHeight) {
	...
	var adjustViewport = function(requiredWidth, requiredHeight) {
		if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent)){
			...
			var wanted_width = Math.min(actual_width, min_width);

			var ratio = Math.min(actual_width / min_width, actual_height / min_height);
			console.log(actual_height, actual_width, min_height, min_width, ratio);

			//document.querySelector('meta[name="viewport"]').setAttribute('content', 'initial-scale=' + ratio + ', maximum-scale=' + ratio + ', minimum-scale=' + ratio + ', user-scalable=yes, width=' + actual_width);
			document.querySelector('meta[name="viewport"]').setAttribute('content', 'initial-scale=' + ratio + ', maximum-scale=' + ratio + ', minimum-scale=' + ratio + ', user-scalable=no, width=' + wanted_width);
			//document.querySelector('meta[name="viewport"]').setAttribute('content', 'initial-scale=' + ratio + ', maximum-scale=' + ratio + ', minimum-scale=' + ratio + ', user-scalable=no, width=440');
		}    
	};
